...
export default () => {
  ...
  const createProfileImage = (url: string, username: string) => `
    <picture class="profile-pic">
      <source type="image/webp" data-srcset="${url}-128.webp 128w, ${url}-256.webp 256w" data-sizes="auto"/>
      <source type="image/jpeg" data-srcset="${url}-128.jpg 128w, ${url}-256.jpg 256w" data-sizes="auto"/>
      <img class="profile-pic lazyload" data-src="${url}-128.png" alt="${username}'s profile picture" width="40px" height="40px"/>
    </picture>
  `;
  ...
  const generateProfile = (profileData: any) => {
    ...
    const avatarsIOPic = `https://avatars.io/twitter/${profileData.social_links.twitter}`;
    const profilePic = (profileData.profile_pic && profileData.profile_pic.length) ? createProfileImage(profileData.profile_pic, profileData.username) :
      `<img class="profile-pic lazyload" alt="${profileData.username}'s profile picture" data-srcset="${avatarsIOPic}/small 48w, ${avatarsIOPic}/medium 128w, ${avatarsIOPic}/large 256w" width="40px" height="40px" data-sizes="auto"></img>`;
    listItem.innerHTML = `
      ${profilePic}
      <div class="line-item">
      ...
      if (id && id.length)
        return `<a href="${type.link(id)}" class="social-icon" target="_blank" rel="noopener">
                    ${logoMap[type.name]}
                  </a>`;
    }).join('')}
      </div>
  ...
  // Divider
  profileList.append(document.createElement('hr'));
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const videoPlaceholder = $('video-placeholder') as HTMLImageElement;
  ...
  for (let i = 0; i < artistFrames.length; i++) {
    ...
    for (let j = 0; j < artistFrames[i].num_video_frames; j++) {
      ...
      image.id = 'video';
      image.idx = imageIdx;
      image.placeholder = artistFrames[i].img_placeholder;
  ...
  const renderFrame = (imageIdx: number, source?: String) => {
    ...
    requestAnimationFrame(async () => {
      ...
      videoPlaceholder.src = videoFrames[imageIdx].image.placeholder;
